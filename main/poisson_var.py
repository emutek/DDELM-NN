...
parser.add_argument(
  "--problem", "-p", type=str, default='grf_32_0b_1f',
  choices=[
    'grf_32_0b_1f'
    ],
...
class PoissonVarYKDD(base_class.YKDD):
  ...
  def Lnet_x_single(self, x: Tensor, theta) -> Tensor:
    return -(self.coeff(x) * dd_act(x @ theta['w'] + theta['b']) * (theta['w'] * theta['w']).sum(dim=0) + d_act(x @ theta['w'] + theta['b']) * torch.einsum('dM, d->M', theta['w'], self.dx_coeff(x)))
  
  def fluxnet_x_single(self, x: Tensor, theta, flux_info: Tensor) -> Tensor:
    return self.coeff(x) * d_act(x @ theta['w'] + theta['b']) * torch.einsum('dM, d->M', theta['w'], flux_info)
...
if __name__ == '__main__':
  ...
  if args.save_img_paper:
    ...
    if rank == 0:
      ...
      uses_fem_ref = args.problem in {'grf_32_0b_1f'}
      if uses_fem_ref:
        y_ref = np.load(f'./fem_sols_revision/u0_plot_var_{args.problem}_32_16_{N}_{plot_n}.npy')
