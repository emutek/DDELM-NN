...
parser.add_argument(
  "--problem", "-p", type=str, default='bou_lay',
  choices=[
    'bou_lay'
  ],
...
d_act = vmap(_d_act)
dd_act = vmap(_dd_act)
class DifConYKDD(base_class.YKDD):
  ...
  def Lnet_x_single(self, x: Tensor, theta) -> Tensor:
    return -dd_act(x @ theta['w'] + theta['b']) * (theta['w'] * theta['w']).sum(dim=0) * self.k + \
      d_act(x @ theta['w'] + theta['b']) * torch.einsum('dM, d->M', theta['w'], self.v)
...
def get_problem(problem: str):
  ...
  if problem == 'bou_lay':
    k = torch.tensor(1e-2, dtype=dtype)
    v = torch.tensor([0, 2], dtype=dtype)
    target_exact = lambda x: (2*x[...,0] - 1) * (1 - torch.exp((2*x[...,1]-2)/k)) / (1 - torch.exp(-2/k))
    target_bou = target_exact
    target_rhs = lambda x: zeros(x.shape[:-1], dtype=dtype)
    target_exact_grad = base_class.act_last(jacrev(target_exact), 0, chunk_size=chunk_size)

  return target_rhs, target_bou, target_exact, target_exact_grad, k, v
  
if __name__ == '__main__':
  t1 = time.time()
  target_rhs, target_bou, target_exact, target_exact_grad, k, v = get_problem(args.problem)
  ...
  if args.save_img_paper:
    ...
    if rank == 0:
      ...
      savefig = lambda title: plt.savefig(f'figures/difcon_{args.problem}_{N}_{args.plot_n}_{title}.{save_format}', format=save_format, bbox_inches='tight')
